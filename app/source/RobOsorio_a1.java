...
public class RobOsorio_a1 extends PApplet {
...
public void draw() {
  ...
  strokeWeight(5);
  float zeroXpx = width*.15f;
  float zeroYpx = height*.85f;
  float maxXpx = width*.85f;
  float maxYpx = height*.15f;
  ...
  for (int i = 1; i < colNames.length; i++) {
    ...
    text(colNames[i], currentX, zeroYpx+height*.1f);
  ...
  for (int d = 0; d < allData.length; d++) { //for each datapoint
    for (int i = 1; i < allData[d].data.length; i++) { //starting at second value, "for each column in data..."
      ...
      if (i == 1) {
        if (isAscending[0]) {
          text(prevMin, colXpx[0], zeroYpx+height*.05f);
          text(prevMax, colXpx[0], maxYpx-height*.05f);
        } else {
          text(prevMin, colXpx[0], maxYpx-height*.05f);
          text(prevMax, colXpx[0], zeroYpx+height*.05f);
        }
      }
      if (isAscending[i]) {
        text(currentMin, colXpx[i], zeroYpx+height*.05f);
        text(currentMax, colXpx[i], maxYpx-height*.05f);
      } else {
        text(currentMin, colXpx[i], maxYpx-height*.05f);
        text(currentMax, colXpx[i], zeroYpx+height*.05f);
      }
      float currentValpx;
      float prevValpx;
...
public void mouseReleased() {
  float xVal = width*.15f;
  for (int i = 0; i < numBars; i++) {
    if (mouseX > xVal-5 && mouseX < xVal+5 && mouseY > height*.15f && mouseY<height*.85f) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "RobOsorio_a1" };
    if (passedArgs != null) {
